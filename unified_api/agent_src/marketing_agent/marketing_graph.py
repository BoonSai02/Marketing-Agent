...
def master_router(state: AgentState) -> str:
    ...
    if state.get("satisfaction") and not state.get("email_sent"):
        return "send_email" # Route to email instead of END
...
def route_after_satisfaction(state: AgentState) -> str:
    """Routes after checking satisfaction."""
    if state.get("satisfaction") and not state.get("email_sent"):
        return "send_email"
    elif state.get("selected_strategy") is None:
        # User wants to pick another strategy
        return "select_strategy" # Go back to selection
    else:
        return END
